...
async fn submit_request(
    ...
    let input_stream = state
        ...
        .await
        .map_err(|error| OpenAIError::internal(format!("open input stream: {error}")))?;
    ...
    state
        ...
        .await
        .map_err(|error| OpenAIError::internal(format!("push input frame: {error}")))?;

    state
        .kernel
        .close_stream(&input_stream.stream_id, None, None, Attributes::new())
        .await
        .map_err(|error| OpenAIError::internal(format!("close input stream: {error}")))?;
